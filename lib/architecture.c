...
/* from basic/string-table.h */

Architecture string_table_lookup(const char * const *table, size_t len, const char *key);
...
#define _DEFINE_STRING_TABLE_LOOKUP_FROM_STRING(name,type,scope)        \
        Architecture name##_from_string(const char *s) {                  \
                return string_table_lookup(name##_table, ELEMENTSOF(name##_table), s); \
        }
